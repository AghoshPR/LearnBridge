...
const TeacherQACommunity = () => {
  ...
  const totalPagesUnanswered = Math.ceil(
    unansweredQuestionsFull.length / itemsPerPage,
  );
  ...
  const totalPagesAnswered = Math.ceil(
    answeredQuestionsFull.length / itemsPerPage,
  );
        ...
        <div className="max-w-6xl mx-auto mt-12 md:mt-0">
          ...
          <div className="space-y-4">
            {activeTab === "unanswered" &&
              currentUnanswered.map((question) => (
                <div
                  key={question.id}
            ...
            {activeTab === "unanswered" &&
              totalPagesUnanswered > 0 &&
              currentUnanswered.length > 0 && (
                <div className="flex justify-center items-center gap-2 mt-6">
                  <button
                    disabled={currentPageUnanswered === 1}
                    onClick={() =>
                      setCurrentPageUnanswered((prev) => Math.max(prev - 1, 1))
                    }
                  ...
                  <button
                    ...
                    onClick={() =>
                      setCurrentPageUnanswered((prev) =>
                        Math.min(prev + 1, totalPagesUnanswered),
                      )
                    }
                    className="px-3 py-1.5 rounded-lg border border-slate-700 text-slate-400 text-sm disabled:opacity-50 hover:bg-slate-800 transition-colors"
                  >
                ...
                >
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-slate-400 text-sm mb-4 line-clamp-2">
                        {question.body || question.description}
                      </p>
              ...
              currentAnswered.length > 0 && (
                <div className="flex justify-center items-center gap-2 mt-6">
                  <button
                    ...
                    onClick={() =>
                      setCurrentPageAnswered((prev) => Math.max(prev - 1, 1))
                    }
                    className="px-3 py-1.5 rounded-lg border border-slate-700 text-slate-400 text-sm disabled:opacity-50 hover:bg-slate-800 transition-colors"
                  >
                    Prev
                  ...
                  <button
                    ...
                    onClick={() =>
                      setCurrentPageAnswered((prev) =>
                        Math.min(prev + 1, totalPagesAnswered),
                      )
                    }
                    className="px-3 py-1.5 rounded-lg border border-slate-700 text-slate-400 text-sm disabled:opacity-50 hover:bg-slate-800 transition-colors"
                  >
            ...
            <div className="p-6">
              <div className="bg-gray-50 rounded-xl p-6 mb-6">
                ...
                <p className="text-gray-600 text-sm mb-4">
                  {selectedQuestion.body || selectedQuestion.description}
                </p>
      ...
      {isAnswersModalOpen && selectedQuestion && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl w-full max-w-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 my-8">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h2 className="text-xl font-bold text-gray-900">
                Answers to: {selectedQuestion.title}
              </h2>
              <button
                onClick={() => {
