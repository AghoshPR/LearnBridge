...
const TeacherManageCourses = () => {
  ...
  const handleAddLesson = async () => {
    ...
    try {
      ...
      await Api.post(`/courses/teacher/courses/${id}/lessons/`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  const handleUpdateLesson = async () => {
    ...
    try {
      ...
      await Api.put(
        `/courses/teacher/lessons/${selectedLesson.id}/`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        },
        ...
        <div className="">
          ...
          <div className="space-y-3">
            {lessons.map((lesson, index) => (
              <div
                key={lesson.id}
                className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-slate-700 transition-colors group"
              >
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-blue-500 transition-colors resize-none"
              ></textarea>
            </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg shadow-2xl scale-100 animate-in zoom-in-95 duration-200 overflow-hidden">
        ...
        <div className="flex items-center justify-end gap-3 p-5 border-t border-slate-800 bg-slate-950/50">
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl text-slate-300 font-medium hover:bg-slate-800 transition-colors text-sm"
          >
            Cancel
          </button>
