...
const TeacherDashBoard = () => {
      ...
      try {
        ...
        toast.error("Failed to load dashboard data", {
          description:
            ...
            "An error occurred fetching dashboard stats.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const stats = [
    ...
    {
      title: "Live Classes",
      value: dashboardData.live_classes_count,
      icon: Video,
      color: "bg-purple-600",
    },
    {
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              {stats.map((stat, index) => (
                <div
                  key={index}
                  className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl hover:border-slate-700 transition-all duration-300 group"
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div
                      className={`p-2 rounded-lg ${stat.color} bg-opacity-20 text-white group-hover:scale-110 transition-transform`}
                    >
                      <stat.icon size={18} />
                    </div>
                  </div>
