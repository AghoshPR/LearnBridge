...
const AdminTeachers = () => {
  ...
  const confirmBlockAction = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || "Failed to block teacher");
    }
  };
  ...
  const confirmDeleteAction = async () => {
    ...
    try {
      ...
      setApproveTeacher(
        approvedTeacher.filter((t) => t.id !== teacherToDelete.id),
      );
      setPendingTeacher(
        pendingTeachers.filter((t) => t.id !== teacherToDelete.id),
      );
  ...
  const filteredApprovedTeachers = approvedTeacher.filter(
    (teacher) =>
      (teacher.name &&
        teacher.name.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (teacher.email &&
  ...
  const totalPagesPending = Math.ceil(
    filteredPendingTeachers.length / itemsPerPage,
  );
  ...
  const totalPagesApproved = Math.ceil(
    filteredApprovedTeachers.length / itemsPerPage,
  );
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-[#111216] border border-gray-800 rounded-2xl overflow-hidden">
            <div className="p-6 border-b border-gray-800">
              <h2 className="text-lg font-bold text-white">
                Approved Teachers {approvedTeacher.length}
              </h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
            ...
            {totalPagesApproved > 0 && filteredApprovedTeachers.length > 0 && (
              <div className="flex justify-center items-center gap-2 p-4 border-t border-gray-800">
                <button
                  ...
                  onClick={() =>
                    setCurrentPageApproved((prev) => Math.max(prev - 1, 1))
                  }
                  className="px-3 py-1.5 rounded-lg border border-gray-700 text-gray-400 text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-800 transition-colors"
                >
                  Prev
                ...
                <button
                  ...
                  onClick={() =>
                    setCurrentPageApproved((prev) =>
                      Math.min(prev + 1, totalPagesApproved),
                    )
                  }
                  className="px-3 py-1.5 rounded-lg border border-gray-700 text-gray-400 text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-800 transition-colors"
                >
          ...
          <div className="bg-[#181a20] rounded-2xl border border-gray-700 w-full max-w-md p-6 relative z-10 shadow-2xl">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-gray-400 mb-6">
                ...
                <span className="text-white font-semibold">
                  {teacherToApprove.name}
                </span>
                ? They will get access to teacher features.
              </p>
              <div className="flex items-center gap-3 w-full">
                <button
                  onClick={() => setTeacherToApprove(null)}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(teacherToView.subjects || "")
                      .split(",")
                      .map((subject, idx) => (
                        <span
                          key={idx}
                          className="bg-gray-800 text-gray-300 px-2 py-0.5 rounded-full text-[10px] border border-gray-700"
                        >
                          {subject.trim()}
                        </span>
                      ))}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="bg-[#0F1014] p-3 rounded-lg border border-gray-800 text-gray-300 text-xs leading-relaxed">
                    <p>
                      {teacherToView.qualification ||
                        "Bachelor of Science in Computer Science, Master of Computer Applications (MCA)"}
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-[#0F1014] border border-gray-800 rounded-xl w-full hover:border-blue-500/30 transition-colors group">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-500">PDF File</p>
                      </div>
                    </div>
