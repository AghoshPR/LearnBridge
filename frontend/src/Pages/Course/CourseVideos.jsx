...
const CourseVideos = () => {
                  ...
                  <div className="absolute right-0 mt-1 bg-white shadow-lg border border-gray-100 rounded-lg p-1 z-20 w-24 overflow-hidden">
                    {isOwner ? (
                      <button
                        ...
                        className="w-full text-left text-xs text-red-600 hover:bg-red-50 p-2 rounded flex items-center gap-2 transition-colors"
                      >
  ...
  const loadReviews = () => {
    Api.get(`/courses/reviews/${courseId}/`).then((res) =>
      setReviews(res.data),
    );
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              {activeLesson?.title} • Lesson {currentLessonIndex} of{" "}
              {totalLessons}
            </p>
          </div>
          <button
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              ...
              <div className="grid gap-6">
                {reviews.map((review) => (
                  <div
                    key={review.id}
                    className="border-b border-gray-50 last:border-0 pb-6 last:pb-0"
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full ${review.color} flex items-center justify-center font-bold text-sm shrink-0`}
                        >
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex flex-col gap-3">
                ...
                <p className="text-xs text-gray-500">
                  {lessons.length} lessons • {calculateTotalDuration(lessons)}{" "}
                  total
                </p>
              </div>
                ...
                {lessons.map((video) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col flex-1 min-w-0">
                      <h4
                        className={`text-sm font-medium leading-tight mb-1 truncate ${video.active ? "text-blue-700" : "text-gray-800"}`}
                      >
                        {video.title}
                      </h4>
                      <span className="text-xs text-gray-500 mb-auto">
